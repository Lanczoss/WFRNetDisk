...
void CloudiskServer::loadUserRegisterModule() {
  _httpserver.POST("/user/signup", [](const HttpReq *req, HttpResp *resp,
                                      SeriesWork *series) {
    if (req->content_type() == APPLICATION_URLENCODED) {
      ...
      // 将用户信息存储到数据库MySQL中
      string mysqlurl("mysql://root:1423@localhost");
...
void CloudiskServer::loadUserLoginModule() {
  _httpserver.POST("/user/signin", [](const HttpReq *req, HttpResp *resp,
                                      SeriesWork *series) {
    if (req->content_type() == APPLICATION_URLENCODED) {
      // 1. 解析请求, 获取用户名和密码
      auto formKV = req->form_kv();
          ...
          mysqlurl, 1, [=](WFMySQLTask *mysqltask) {
            ...
            if (cursor.get_cursor_status() == MYSQL_STATUS_OK) {
              ...
              if (encodedPassword == M) {
                ...
                // 3.3 将Token保存到数据库中
                auto nextTask =
                    WFTaskFactory::create_mysql_task(mysqlurl, 1, nullptr);
                string sql(
                    "REPLACE INTO cloudisk.tbl_user_token(user_name, "
                    "user_token)VALUES('");
...
void CloudiskServer::loadFileQueryModule() {
  _httpserver.POST("/file/query", [](const HttpReq *req, HttpResp *resp,
    ...
    string sql(
        ...
        "cloudisk.tbl_user_file where user_name = '");
...
void CloudiskServer::loadFileUploadModule() {
  _httpserver.POST("/file/upload", [](const HttpReq *req, HttpResp *resp,
    ...
    if (req->content_type() == MULTIPART_FORM_DATA) {
      ...
      string sql(
          "INSERT INTO "
          "cloudisk.tbl_user_file(user_name,file_sha1,file_size,file_name)"
          "VALUES('");
...
void CloudiskServer::loadFileDownloadModule() {
  _httpserver.GET("/file/downloadurl", [](const HttpReq *req, HttpResp *resp) {
    ...
    // 这要求我们还需要去部署一个下载服务器
    string downloadURL = "http://172.28.97.159:8080/" + filename;
